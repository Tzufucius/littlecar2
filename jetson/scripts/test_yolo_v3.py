...
# ==================== 配置区域 ====================
# 建议在此处将权重文件路径指向您新训练的 7 类模型
MODEL_PATH = PROJECT_ROOT / "assets" / "models" / "6color-circle-v3.pt" 
DATASET_DIR = PROJECT_ROOT / "assets" / "彩色物料数据集v3"
DEVICE = "cuda:0" if cv2.ocl.haveOpenCL() else "cpu"
CONF_THRES = 0.5
IOU_THRES = 0.45
NUM_TEST_IMAGES = 200  # 压测样本数

# 映射类别 - 新增了第7类 "EmptySlot" (索引为6)
CLASS_NAMES = ["Red", "Yellow", "Blue", "Green", "Black", "LightBlue", "EmptySlot"]

# 用于图表美观的近似物理颜色对应 - 新增了 "dimgray" 用于代表空心圆物料空位
CHART_COLORS = ["crimson", "gold", "royalblue", "forestgreen", "black", "skyblue", "dimgray"]
# ==================================================
...
def main() -> None:
    ...
    # 初始化统计数据 - 基于 len(CLASS_NAMES) 动态分配长度
    num_classes = len(CLASS_NAMES)
    total_counts = [0] * num_classes    # 每种颜色/类型真实出现的总次数
    class_errors = [0] * num_classes    # 每种颜色/类型定位到了但分类判断错误的次数
    missed_counts = [0] * num_classes   # 每种颜色/类型完全漏检未检出的次数
    ...
    for i, path_str in enumerate(selected_img_paths, 1):
        ...
        if i % 10 == 0 or i == test_count:
            print(f"进度：已推理并处理 {i}/{test_count} 张图像...")
    ...
    ax1.set_title("Ground Truth Class Distribution", fontsize=11, fontweight='bold')
    ax1.set_xlabel("Material Class")
    ax1.set_ylabel("Total Occurrences")
    ax1.grid(axis='y', linestyle='--', alpha=0.5)
    plt.setp(ax1.get_xticklabels(), rotation=15, ha="right") # 旋转字体防止7分类名字重叠
    ...
    ax2.set_title("Failures Distribution per Class", fontsize=11, fontweight='bold')
    ...
    plt.suptitle("Batch Performance & Failures Diagnostics (7-Class Scheme)", fontsize=14, y=0.98)
    plt.tight_layout()
    plt.show()  # 弹出统计图窗口
    ...
    if wrong_examples:
        ...
        print("🎉 检测分析完毕：抽样样本表现非常优异，未发现任何分类错误或漏检目标！")
